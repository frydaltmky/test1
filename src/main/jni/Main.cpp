...
{
    static int Tab = 1;
    static bool TestingMode = false; // خيار الاختبار الجديد
}
...
inline EGLBoolean hook_eglSwapBuffers(EGLDisplay dpy, EGLSurface surface) {
    ...
    // نافذة جديدة بسيطة فيها خيار واحد فقط
    ImGui::SetNextWindowSize(ImVec2((float) g_GlWidth * 0.30f, (float) g_GlHeight * 0.20f), ImGuiCond_Once);
    if (ImGui::Begin("Zygisk [ x32/x64 ]", nullptr, ImGuiWindowFlags_NoBringToFrontOnFocus | ImGuiWindowFlags_NoCollapse)) {
        ImGui::TextWrapped("Simple Menu");
        ImGui::Spacing();
        ImGui::Spacing();

        ImGui::Checkbox("Testing in progress", &Settings::TestingMode);
        ImGui::SameLine();
